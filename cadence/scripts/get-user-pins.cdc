...
access(all) fun main(address: Address): [PinMetadata] {
    ...
    // Check if the capability exists first
    let collectionCap = account.capabilities.get<&Pinnacle.Collection>(
            Pinnacle.CollectionPublicPath
        )

    // If it doesn't exist, or can't be borrowed, return an empty list
    if !collectionCap.exists() || collectionCap.borrow() == nil {
        return pins
    }

    // If it does exist, borrow it and continue
