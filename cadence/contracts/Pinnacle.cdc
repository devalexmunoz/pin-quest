...
///
access(all) contract Pinnacle: NonFungibleToken {
    //------------------------------------------------------------
    // Events
    //------------------------------------------------------------
    ...
    access(all) event SeriesCreated(id: Int, name: String)
    /// Emitted when a Series is locked, meaning new Sets cannot be created with the Series anymore
    access(all) event SeriesLocked(id: Int, name: String)
    /// Emitted when a Series's name is updated
    access(all) event SeriesNameUpdated(id: Int, name: String)

    // Set Events
    //
    /// Emitted when a new Set has been created, meaning new Shapes can be created with the Set
    access(all) event SetCreated(id: Int, renderID: String, name: String, seriesID: Int, editionType: String)
    /// Emitted when a Set is locked, meaning new Shapes cannot be created with the Set anymore
    access(all) event SetLocked(id: Int, renderID: String, name: String, seriesID: Int, editionType: String)
    /// Emitted when a Set's name is updated
    access(all) event SetNameUpdated(id: Int, renderID: String, name: String, seriesID: Int, editionType: String)

    // Shape Events
    //
    /// Emitted when a new Shape has been created, meaning new Editions can be created with the Shape
    access(all) event ShapeCreated(
        id: Int,
        renderID: String,
        setID: Int,
        name: String,
    ...
    access(all) event ShapeNameUpdated(
        id: Int,
        renderID: String,
        setID: Int,
        name: String,
    ...
    /// Emitted when a Shape's metadata field is updated
    access(all) event ShapeMetadataUpdated(
        id: Int,
        metadata: {String: [String]}
    )
    /// Emitted when a Shape's current printing is incremented
    access(all) event ShapeCurrentPrintingIncremented(
        id: Int,
        renderID: String,
        setID: Int,
        name: String,
    ...
    access(all) event EditionClosed(
        id: Int,
        maxMintSize: UInt64
    )
    /// Emitted when an Edition's description is updated
    access(all) event EditionDescriptionUpdated(
        id: Int,
        description: String
    )

    /// Emitted when an Edition's renderID is updated
    access(all) event EditionRenderIDUpdated(
        id: Int,
        renderID: String
    )
    ...
    access(all) event EditionRemoved(
        id: Int
    )
    ...
    access(all) event EditionTypeCreated(id: Int, name: String, isLimited: Bool, isMaturing: Bool)
    /// Emitted when an Edition Type has been closed, meaning new Editions cannot be created with the Edition
    /// Type anymore
    access(all) event EditionTypeClosed(id: Int, name: String, isLimited: Bool, isMaturing: Bool)
    ...
    access(all) event PinNFTMinted(id: UInt64, renderID: String, editionID: Int, serialNumber: UInt64?, maturityDate: UInt64?)
    /// Emitted when a Pin NFT's XP is updated
    access(all) event NFTXPUpdated(id: UInt64, editionID: Int, xp: UInt64?)
    /// Emitted when an Inscription is added to a Pin NFT
    access(all) event NFTInscriptionAdded(
        id: Int,
        owner: Address,
        note: String?,
        nftID: UInt64,
    ...
    access(all) event NFTInscriptionUpdated(
        id: Int,
        owner: Address,
        note: String?,
        nftID: UInt64,
    ...
    access(all) event NFTInscriptionRemoved(id: Int, owner: Address, nftID: UInt64, editionID: Int)
    ...
    access(all) event EntityReactivated(entity: String, id: Int, name: String?)
    /// Emitted when a new Variant has been inserted
    access(all) event VariantInserted(name: String)
    /// Emitted when the wrapper emitPurchasedEvent Admin function is called
    access(all) event Purchased(
    ...
    access(all) event OpenEditionNFTBurned(id: UInt64, editionID: Int)
    ...
    access(all) struct Series {
        ...
        access(all) var lockedDate: UInt64?

        /// Struct initializer
        ///
        view init(id: Int, name: String) {
            self.id = id
            self.name = name
            self.lockedDate = nil
        }
    ...
    ///
    access(all) view fun getLatestSeriesID(): Int {
    ...
    /// contract
    ///
    access(all) view fun getSeries(id: Int): Series? {
        pre {
    ...
    ///
    access(all) view fun getAllSeries(): [Series] {
    ...
    /// contract
    ///
    access(all) view fun getSeriesByName(_ name: String): Series? {
    ...
    access(all) struct Set {
        ...
        /// Struct initializer
        ///
        view init(id: Int, renderID: String, name: String, editionType: String, seriesID: Int) {
            self.id = id
        ...
        ///
        access(contract) view fun shapeNameExistsInSet(_ name: String): Bool {
        ...
        ///
        access(all) view fun getShapeNames(): {String: Bool} {
    ...
    ///
    access(all) view fun getLatestSetID(): Int {
    ...
    ///
    access(all) view fun getSet(id: Int): Set? {
        pre {
    ...
    access(all) struct Shape {
        ...
        access(all) var closedDate: UInt64?
        ...
        access(all) var currentPrinting: UInt64
        ...
        ///
        access(contract) view fun variantPrintingPairExistsInEdition(_ variant: String): Bool {
        ...
        /// Set this Shape's metadata field
        ///
        access(contract) fun setMetadataFields(_ metadata: {String: AnyStruct}) {
            pre {
                // Check that metadata is valid
                (metadata.containsKey("Franchises") == false
                    || metadata["Franchises"]!.isInstance(Type<[String]>())):
                    "Franchises must be a string array"
                (metadata.containsKey("Studios") == false || metadata["Studios"]!.isInstance(Type<[String]>())):
                    "Studios must be a string array"
                (metadata.containsKey("Categories") == false || metadata["Categories"]!.isInstance(Type<[String]>())): 
                    "Categories must be a string array"
                (metadata.containsKey("RoyaltyCodes") == false ||
            ...
            for key in self.metadata.keys {
                convertedMetadata[key] = self.metadata[key]!.isInstance(Type<String>()) == true ?
                    [self.metadata[key]! as! String] : self.metadata[key]! as! [String]
            }

            emit ShapeMetadataUpdated(id: self.id, metadata: convertedMetadata)
        }
        ...
        ///
        access(all) view fun getVariantPrintingPairsInEditions(): {String: UInt64} {
    ...
    ///
    access(all) view fun getLatestShapeID(): Int {
    ...
    ///
    access(all) view fun getShape(id: Int): Shape? {
        pre {
    ...
    ///
    access(all) view fun getAllShapes(): [Shape] {
    ...
    access(all) struct Edition {
        ...
        access(all) let editionTypeID: Int
        ...
        /// new NFT is minted
        access(all) var numberMinted: UInt64
        ...
        access(all) let maturationPeriod: UInt64?
        ...
        access(all) let creationDate: UInt64
        ...
        ///
        access(all) view fun isMaxEditionMintSizeReached(): Bool {
        ...
        ///
        access(contract) view fun getNextSerialNumber(): UInt64? {
    ...
    ///
    access(all) view fun getLatestEditionID(): Int {
    ...
    /// contract
    ///
    access(all) view fun getEdition(id: Int): Edition? {
        pre {
    ...
    ///
    access(all) view fun getAllEditions(): [Edition] {
    ...
    access(all) struct EditionType {
        ...
        access(all) let isMaturing: Bool
        ...
        /// Struct initializer
        ///
        view init(id: Int, name: String, isLimited: Bool, isMaturing: Bool) {
            self.id = id
            self.name = name
    ...
    ///
    access(all) view fun getLatestEditionTypeID(): Int {
    ...
    /// in the contract
    ///
    access(all) view fun getEditionType(id: Int): EditionType? {
        pre {
    ...
    ///
    access(all) view fun getAllEditionTypes(): [EditionType] {
    ...
    /// in the contract
    ///
    access(all) view fun getEditionTypeByName(_ name: String): EditionType? {
    ...
    ///
    access(all) view fun getEditionTypeIDByName(_ name: String): Int? {
    ...
    access(all) struct Inscription {
        ...
        ///
        access(all) view fun getExtension(): {String: AnyStruct} {
    ...
    access(all) resource NFT: NonFungibleToken.NFT {
        ...
        /// Event emitted when a Pin NFT is destroyed (replaces PinNFTBurned event in Pinnacle contract
        /// before Cadence 1.0 update)
        ///
        access(all) event ResourceDestroyed(
            id: UInt64 = self.id,
            editionID: Int = self.editionID,
        ...
        ///
        access(all) view fun getMaturityDate(): UInt64? {
        ...
        ///
        access(all) view fun isLocked(): Bool {
        ...
        ///
        access(all) view fun getInscriptionsLimit(): Int {
        ...
        ///
        access(contract) view fun borrowInscription(id: Int): &Inscription? {
            pre {
        ...
        access(contract) fun updateCurrentOwnerInscriptionNote(currentOwner: Address, note: String) {
            ...
            Pinnacle.emitNFTUpdated(&self as auth(NonFungibleToken.Update) &NFT)
        }
        ...
        ///
        access(all) view fun getLatestInscriptionID(): Int {
        ...
        /// exists in the NFT
        ///
        access(all) view fun getInscription(id: Int): Inscription? {
            pre {
                id > 0: "The ID of an Inscription must be greater than 0"
            }
        ...
        ///
        access(all) view fun getAllInscriptions(): [Inscription] {
        ...
        /// exists in the NFT
        ///
        access(all) view fun getInscriptionByAddress(_ address: Address): Inscription? {
        ...
        ///
        access(all) view fun getInscriptionIDsByAddress(_ address: Address): Int? {
            return self.inscriptionIDsByAddress[address]
        }

        /// Return this NFT's inscriptionIDsByAddress dictionary
        ///
        access(all) view fun getAllInscriptionIDsByAddresses(): {Address: Int} {
        ...
        ///
        access(all) fun resolveDisplayView(): MetadataViews.Display {
        ...
        ///
        access(all) fun resolveExternalURLView(): MetadataViews.ExternalURL {
        ...
        ///
        access(all) fun resolveMediasView(): MetadataViews.Medias {
        ...
        ///
        access(all) fun resolveEditionsView(): MetadataViews.Editions {
            let edition = Pinnacle.getEdition(id: self.editionID)!
            let shape = Pinnacle.getShape(id: edition.shapeID)!
            let set = Pinnacle.getSet(id: edition.setID)!
            // Assemble the name
        ...
        ///
        access(all) fun resolveSerialView(): MetadataViews.Serial? {
        ...
        ///
        access(all) fun resolveRoyaltiesView(): MetadataViews.Royalties {
            let royaltyReceiver: Capability<&{FungibleToken.Receiver}> =
                getAccount(Pinnacle.royaltyAddress).capabilities.get<&{FungibleToken.Receiver}>(
        ...
        ///
        access(all) fun getThumbnailPath(): MetadataViews.HTTPFile {
            return MetadataViews.HTTPFile(url:"https://assets.disneypinnacle.com/on-chain/pinnacle.jpg")
        }

        /// Return an asset path
        ///
        access(all) view fun getAssetPath(): String {
            return "placeholder_pinnacle_base_asset_path"
        }

        /// Return an image path
        ///
        access(all) view fun getImagePath(): String {
            return "placeholder_image_path"
        }

        /// Return a video path
        ///
        access(all) view fun getVideoPath(): String {
            return "placeholder_video_path"
        }

        /// Return a Pin path
        ///
        access(all) view fun getPinPath(): String {
        ...
        access(all) fun createEmptyCollection(): @{NonFungibleToken.Collection} {
            return <- Pinnacle.createEmptyCollection(nftType: Type<@NFT>())
        }
    }
    ...
    // Deprecated in favor of using entitlement-based access control as required by Cadence 1.0
    access(all) resource interface PinNFTCollectionPublic: NonFungibleToken.Collection {}

    /// Resource that defines a Pinnacle NFT Collection
    ///
    access(all) resource Collection: NonFungibleToken.Collection, PinNFTCollectionPublic {
        ...
        access(NonFungibleToken.Withdraw) fun withdraw(withdrawID: UInt64): @{NonFungibleToken.NFT} {
            ...
            emit Withdraw(id: nft.id, from: self.owner?.address)  // TODO: Consider removing
            return <- nft
        }
        ...
        ///
        access(NonFungibleToken.Update) fun addNFTInscription(id: UInt64): Int {
            pre {
        ...
        /// current owner's.
        ///
        access(NonFungibleToken.Update) fun removeNFTInscription(id: UInt64) {
        ...
        ///
        access(NonFungibleToken.Update) fun updateNFTInscriptionNote(id: UInt64, note: String, adminRef: &Admin) {
        ...
        ///
        access(NonFungibleToken.Update) fun toggleNFTXP(id: UInt64): UInt64? {
            return self.borrowPinNFT(id: id)!.toggleXP()
        }

        /// Activate or deactivate the XP of all the NFTs in this Collection
        ///
        access(NonFungibleToken.Update) fun batchToggleXP(_ activateAll: Bool) {
            // Iterate through the NFT IDs in the Collection
        ...
        /// This function returns nil if the NFT does not exist in this Collection.
        ///
        /// This function exposes all the NFT's fields and functions, though there are functions that
        ...
        ///
        access(all) view fun borrowPinNFT(id: UInt64): &NFT? {
            return self.borrowNFT(id) as! &NFT?
        }

        /// Return a reference to an NFT in this Collection typed as ViewResolver.Resolver
        ///
    ...
    ///
    access(all) view fun borrowCollectionPublic(owner: Address, _ publicPathID: String?): &Collection? {
        return getAccount(owner).capabilities.borrow<&Collection>(
    ...
    /// Entitlement that grants the ability to mint Pinnacle NFTs
    ...
    access(all) resource Admin: NFTMinter {
        ...
        /// (no undo period).
        ///
        access(Operate) fun createSeries(name: String): Int {
            pre {
                name != "": "The name of a Series cannot be an empty string"
                Pinnacle.seriesIDsByName.containsKey(name) == false: "A Series with that name already exists"
            }
        ...
        ///
        access(self) view fun borrowSeries(id: Int): &Series? {
            pre {
                id > 0: "The ID of a Series must be greater than 0"
            }
        ...
        ///
        access(Operate) fun lockSeries(id: Int, undo: Bool) {
            undo ? self.borrowSeries(id: id)!.unlock() : self.borrowSeries(id: id)!.lock()
        }

        /// Update a Series's name
        ///
        access(Operate) fun updateSeriesName(id: Int, name: String) {
        ...
        /// (no undo period).
        ///
        access(Operate) fun createSet(renderID: String, name: String, editionType: String, seriesID: Int): Int {
            pre {
        ...
        ///
        access(self) view fun borrowSet(id: Int): &Set? {
            pre {
        ...
        ///
        access(Operate) fun lockSet(id: Int, undo: Bool) {
            undo ? self.borrowSet(id: id)!.unlock() : self.borrowSet(id: id)!.lock()
        }

        /// Update a Set's name
        ///
        access(Operate) fun updateSetName(id: Int, name: String) {
        ...
        /// (no undo period).
        ///
        access(Operate) fun createShape(
            renderID: String,
            setID: Int,
            name: String,
            ...
            pre {
                ...
                (metadata.containsKey("Studios") == true || metadata.containsKey("Categories") == true):
                    "Either Studios or Categories must be present"
                (metadata.containsKey("Studios") == false || metadata["Studios"]!.isInstance(Type<[String]>())):
                    "Studios must be a string array"
                (metadata.containsKey("Categories") == false || metadata["Categories"]!.isInstance(Type<[String]>())): 
                    "Categories must be a string array"
        ...
        ///
        access(self) view fun borrowShape(id: Int): &Shape? {
            pre {
        ...
        ///
        access(Operate) fun closeShape(id: Int, undo: Bool) {
            undo ? self.borrowShape(id: id)!.reopen() : self.borrowShape(id: id)!.close()
        }

        /// Update a Shape's name
        ///
        access(Operate) fun updateShapeName(id: Int, name: String) {
            let shapeRef = self.borrowShape(id: id)!
            shapeRef.updateName(name, self.borrowSet(id: shapeRef.setID)!)
        }

        /// Set a Shape's metadata fields
        ///
        access(Operate) fun setShapeMetadataFields(id: Int, metadata: {String: AnyStruct}) {
            let shapeRef = self.borrowShape(id: id)!
            shapeRef.setMetadataFields(metadata)
        }

        /// Increment a Shape's current printing
        ///
        access(Operate) fun incrementShapeCurrentPrinting(id: Int): UInt64 {
        ...
        ///
        access(self) view fun borrowEdition(id: Int): &Edition? {
            pre {
        ...
        ///
        access(Operate) fun closeEdition(id: Int) {
        ...
        ///
        access(Operate) fun removeEdition(id: Int) {
        ...
        ///
        access(Operate) fun reopenEdition(id: Int) {
            self.borrowEdition(id: id)!.reopen()
        }

        /// Update an Edition's description
        ///
        access(Operate) fun updateEditionDescription(id: Int, description: String) {
            self.borrowEdition(id: id)!.updateDescription(description)
        }

        /// Update an Edition's renderID
        ///
        access(Operate) fun updateEditionRenderID(id: Int, renderID: String) {
        ...
        /// (no undo period).
        ///
        access(Operate) fun createEditionType(name: String, isLimited: Bool, isMaturing: Bool): Int {
            pre {
        ...
        ///
        access(self) view fun borrowEditionType(id: Int): &EditionType? {
            pre {
        ...
        ///
        access(Operate) fun closeEditionType(
            id: Int,
        ...
        ///
        access(Operate) fun reopenEditionType(id: Int) {
        ...
        ///
        access(Operate) fun insertVariant(name: String) {
            pre {
        ...
        ///
        access(Mint) fun mintNFT(editionID: Int, extension: {String: String}?): @NFT {
        ...
        ///
        access(Operate) fun burnOpenEditionNFT(_ nft: @{NonFungibleToken.NFT}) {
            let nft <- nft as! @NFT
        ...
        /// ID (default is 100)
        ///
        access(Operate) fun setNFTInscriptionsLimit(
            nftID: UInt64,
            limit: Int,
            owner: Address,
            ...
            assert(collectionPublicRef!.borrowNFT(nftID) != nil,
        ...
        ///
        access(Operate) fun subtractXPfromNFT(
            nftID: UInt64,
            owner: Address,
            collectionPublicPathID: String?,
            value: UInt64
        ...
        ///
        access(Operate) view fun emitPurchasedEvent(
            purchaseIntentID: String,
            buyerAddress: Address,
            countPurchased: UInt64,
            totalSalePrice: UFix64
        ...
        ///
        access(Operate) fun createAdmin(): @Admin {
            return <- create Admin()
        }

        /// Set the contract's royalty address
        ///
        access(Operate) fun setRoyaltyAddress(_ address: Address) {
            Pinnacle.royaltyAddress = address
        }

        /// Set the contract's end user license URL
        ///
        access(Operate) fun setEndUserLicenseURL(_ url: String) {
            Pinnacle.endUserLicenseURL = url
        }

        /// Set an entry in the contract's extension dictionary
        ///
        access(Operate) fun setExtensionEntry(_ key: String, _ value: AnyStruct, _ overwrite: Bool) {
            pre {
    ...
    /// Emit generic Updated event in NonFungibleToken contract interface
    ///
    /// Although the Pinnacle-specific NFTInscriptionUpdated and NFTXPUpdated events allow indexing the details of what is being updated, there is
    /// value in also emitting the generic Updated event as listeners can have support for it by default thanks to being defined in NonFungibleToken v2.
    ///
    access(all) view fun emitNFTUpdated(_ nftRef: auth(NonFungibleToken.Update) &{NonFungibleToken.NFT}) {}

    /// Return the contract's variants dictionary
    ///
    access(all) view fun getAllVariants(): {String: Bool} {
        return Pinnacle.variants
    }

    /// Allow iterating over Variants in the contract without allocating an array
    ///
    access(all) fun forEachVariant(_ function: fun (String): Bool) {
    ...
    ///
    access(all) view fun makeQueueName(bucketName maybeBucketName: String?): String {
        if let bucketName = maybeBucketName {
    ...
    ///
    /// The function relies on checking the type of the imported MetadataViews contract.
    /// 0x1d7e57aa55817448 is the address of the known MetadataViews contract standard on mainnet.
    /// This is a workaround for the fact that there is no way to check the network ID in Cadence yet.
    ///
    access(all) view fun isContractDeployedToMainnet(): Bool {
        return Type<MetadataViews>().identifier == "A.1d7e57aa55817448.MetadataViews"
    }
    ...
    ///
    access(all) fun resolveNFTCollectionDataView(): MetadataViews.NFTCollectionData {
        return MetadataViews.NFTCollectionData(
            ...
            createEmptyCollectionFunction: (fun(): @{NonFungibleToken.Collection} {return <- Pinnacle.createEmptyCollection(nftType: Type<@NFT>())})
        )
    }

    /// Resolve this contract's NFTCollectionDisplay view
    ///
    access(all) fun resolveNFTCollectionDisplayView(): MetadataViews.NFTCollectionDisplay {
    ...
    init(undoPeriod: UInt64) {
        ...
        // Save the Admin resource to storage
